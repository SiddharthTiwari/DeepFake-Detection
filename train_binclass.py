...
def main():
    ...
    parser.add_argument('--faces_df_path', type=str, action='store',
                        help='Path to the Pandas Dataframe obtained from extract_faces.py on the FF++ dataset. '
                             'Required for training/validating on the dataset.')
    ...
    for dataset in train_datasets:
        if df_path is None or faces_dir is None:
            raise RuntimeError('Specify DataFrame and directory for faces for training!')
    for dataset in val_datasets:
        if df_path is None or faces_dir is None:
            raise RuntimeError('Specify DataFrame and directory for faces for validation!')
    # Load splits with the make_splits function
    splits = split.make_splits(faces_df=df_path,faces_dir=faces_dir,
